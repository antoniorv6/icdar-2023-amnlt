...
@gin.configurable
class RecurrentSingleLevel(nn.Module):

    def __init__(self, out_cats, fixed_height=64):
        super(RecurrentSingleLevel, self).__init__()
        self.features = (fixed_height // 32) * 512
        self.dec_lstm = nn.LSTM(input_size=self.features, hidden_size=256, bidirectional=True, batch_first=True)
        self.out_dense = nn.Linear(in_features=512, out_features=out_cats)
    
    def forward(self, x):
        b, c, h, w = x.size()
        x = x.permute(0, 3, 2, 1).contiguous()
        x = x.reshape(b, -1, self.features)
        x, _ = self.dec_lstm(x)
        x = self.out_dense(x)
        return F.log_softmax(x, dim=2)
...
class TransformerScoreUnfolding1D(nn.Module):
    def __init__(self, out_cats, max_len):
        super(TransformerScoreUnfolding1D, self).__init__()
        self.dummy_param = nn.Parameter(torch.empty(0))
...
class E2EScore_CNNT1D(nn.Module):

    def __init__(self, in_channels, out_cats, max_len, pretrain_path=None):
        super(E2EScore_CNNT1D, self).__init__()
        self.encoder = Encoder(in_channels=in_channels)
        self.decoder = TransformerScoreUnfolding1D(out_cats=out_cats, max_len=max_len)
...
def get_CNNT1D_model(in_channels, out_size, mh, mw, maxlen=5000):
    model = E2EScore_CNNT1D(in_channels=in_channels, out_cats=out_size, max_len=maxlen)
    return model
